...
install_deps() {
  if command -v apt-get >/dev/null 2>&1; then
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y \
      python3 nfs-common cifs-utils open-iscsi openssh-client qemu-utils
  elif command -v dnf >/dev/null 2>&1; then
    dnf install -y python3 nfs-utils cifs-utils iscsi-initiator-utils openssh-clients qemu-img
    ...
    echo "Install Python 3, NFS, CIFS, iSCSI, OpenSSH client and qemu-img manually." >&2
    exit 2
  fi
}
...
echo "  python3 nfs-common cifs-utils open-iscsi openssh-client qemu-utils"
echo "Optional connectors:"
echo "  govc    - VMware ESXi/vCenter"
echo "  sshpass - password SSH fallback; SSH keys are preferred"
echo
